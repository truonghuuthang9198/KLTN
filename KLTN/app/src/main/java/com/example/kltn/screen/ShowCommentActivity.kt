...
//        setUpRecyclerview()
    ...
    private fun setViewReviewComment(response: Response<ReviewResponse>) {
        ...
        var sumStar =
            (5 * response.body()!![0].namSao + 4 * response.body()!![0].bonSao + 3 * response.body()!![0].baSao + 2 * response.body()!![0].haiSao + response.body()!![0].motSao) / demcmt.toFloat()
        tvSumStar.text = (Math.round(sumStar * 10.0) / 10.0).toString()
